...
class HistoriqueComposant extends StatefulWidget {
  ...
  @override
  State<HistoriqueComposant> createState() => _HistoriqueComposantState();
}

class _HistoriqueComposantState extends State<HistoriqueComposant> {
  ...
  List<DataRow> buildRows(List<dynamic> data) {
    return data.map((article) {
      ...
      return DataRow(cells: [
        DataCell(Text(article["composant_id"].toString())),
  ...
  Widget build(BuildContext context) {
    ...
    if (composant_modifer != null) {
      DateTime now = DateTime.now();
      String today = DateFormat('yyyy-MM-dd').format(now);
      String yesterday = DateFormat('yyyy-MM-dd')
          .format(now.subtract(const Duration(days: 1)));
      for (var article in composant_modifer!) {
